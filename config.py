...
# ⛔ ОТКЛЮЧЕНО. Шорт спота (маржинальный заём монеты) на практике слил баланс:
# деньги застревали в займах/холдингах на споте, USDT уходил в маржу, откат ноги
# был ненадёжен. Слишком хрупко на живом счёте — оставляем только two long-spot
# направления (long_spot_short_perp + basis_short_perp), которые работают стабильно.
NEGATIVE_FUNDING_ENABLED  = False
MIN_NEGATIVE_FUNDING_RATE = -0.00015  # войти когда rate ≤ -0.015%/8h
EXIT_NEGATIVE_FUNDING_RATE = -0.00006  # выйти когда rate ≥ -0.006%/8h

# ⛔ Шорт спота выключен — см. выше. НЕ включать без полного теста на testnet.
...
# Не входить в basis_short_perp (long spot + short perp) при отрицательном фандинге:
# короткая нога перпа ПЛАТИТ фандинг → двойной минус поверх комиссий за время удержания.
# Именно так слили на MOVEUSDT (rate=−0.016%, вход → converged за 0ч → −fee).
BASIS_BLOCK_NEGATIVE_FUNDING = True

# --- Позиция ---
# 🔬 РЕЖИМ МАЛЫХ СУММ (после двух сливов на mainnet): фиксированный $10/нога,
# 1 позиция максимум. Наблюдаем что long-spot открывается и ЗАКРЫВАЕТСЯ чисто
# на реальном счёте при минимальном риске. Когда убедимся — поднимем размер.
POSITION_USD         = 10.0     # USD на каждую ногу (Kelly выключен → используется это)
MAX_POSITIONS        = 1        # одна позиция за раз (наблюдаем поведение)

# --- Kelly-критерий (динамический размер позиции) ---
# ⛔ ВЫКЛЮЧЕН в режиме малых сумм — размер фиксирован (POSITION_USD).
# Включить обратно (KELLY_ENABLED=True), когда поведение подтвердится на mainnet.
KELLY_ENABLED        = False
